...
void deleteSonginPlaylist(playlist *P){
	...
	if ((*P).songCount == (*P).deletedCount){
		...
		//if ID non-existent, prompt error
		if (songAccess > (*P).songCount){
			printf("Song ID empty!\n");
		}
...
void savePlaylists(FILE *fp, playlist *P, int playlistCount){
	fprintf(fp, "%d\n", playlistCount);
	for (int i = 0; i < playlistCount; i++)
	{
		fprintf(fp, "%d. %s\n", P[i].id +1, P[i].playlist_name); //1. playlist_name
		fprintf(fp, "%d\n", P[i].songCount-P[i].deletedCount);
		for (int j = 0; j < P[i].songCount-P[i].deletedCount; j++){
			if (P[i].songlist[j].songFlag != 0){
				fprintf(fp, "%s %s %s\n",P[i].songlist[j].title, P[i].songlist[j].singer, P[i].songlist[j].album);
			}
		}
	}
}

void loadPlaylists(FILE *fp, playlist *P, int *plSize){
	fscanf(fp, "%d\n", plSize);
	for (int i = 0; i < *plSize; i++){
		fscanf(fp, "%d. %s\n%d\n", &P[i].id, P[i].playlist_name, &P[i].songCount);
		P[i].id --;
		for (int j = 0; j < P[i].songCount; j++){
			P[i].songlist[j].songFlag = 1;
			fscanf(fp, "%s %s %s\n", P[i].songlist[j].title,P[i].songlist[j].singer, P[i].songlist[j].album);
		}
	}
}